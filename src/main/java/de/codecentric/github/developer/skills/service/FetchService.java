...
public class FetchService {
    ...
    public List<Developer> fetch(final String gitHubMemberUrl) {
        ...
        developersResponse.getBody().stream().map(webDeveloper -> {
            final ResponseEntity<List<WebRepository>> repositoriesResponse = new RestTemplate()
                    .exchange(webDeveloper.getRepositoriesUrl(), HttpMethod.GET, null, new ParameterizedTypeReference<>() {});

            repositoriesResponse.getBody().stream().map(webRepository -> {
